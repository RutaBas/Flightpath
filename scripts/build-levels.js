...
/* Tier KEYS are permanent: they are the identity the meta-layer persists, and
   ...
   APPEND-ONLY. New tiers go on the END of this list and nowhere else. The bake
   walks it in order sharing one duplicate-detection set, so inserting a tier in
   the middle would shift every later tier's pool and silently re-cut levels
   players have already cleared. */
const TIERS = [
  { key: "clear", tier: 1 },
  { key: "light", tier: 2 },
  { key: "holding", tier: 3 },
  { key: "stacked", tier: 4 },
  { key: "gridlock", tier: 5 },
  /* Added 2026-08-13. Tiers 1-5 re-bake bit-identically because they are
     searched first, from the same seed 1 upward, against the same pool target
     and the same ramp — verified row by row by scripts/validate-levels.js. */
...
lines.push("");
lines.push("   " + (TIERS.length * LEVELS_PER_TIER) + " levels: " + TIERS.length +
           " tiers x " + LEVELS_PER_TIER + ", each [seed, effort, parMs].");
lines.push("");
lines.push("   ORDER is the ladder's spine: tier KEY (the persisted identity), the");
lines.push("   generator's 1-based tier index, and the grid, all read from the generator's");
lines.push("   own band table at bake time. js/meta-config.js merges display names onto it,");
lines.push("   so nothing in this file has to be edited to rename a tier — and a rename");
lines.push("   cannot reach a storage key, because no name is stored anywhere.");
lines.push("");
...
lines.push("  var ORDER = [");
TIERS.forEach((spec, i) => {
  const band = Generator.tierFor(spec.tier);
  lines.push('    { key: "' + spec.key + '", tier: ' + spec.tier +
...
lines.push("  }");
lines.push("");
lines.push("  function specFor(tierKey) {");
lines.push("    for (var i = 0; i < ORDER.length; i++) if (ORDER[i].key === tierKey) return ORDER[i];");
lines.push("    return null;");
lines.push("  }");
lines.push("");
lines.push("  /* Every level in the ladder — what a totals line divides by, so nothing");
lines.push("     has to hard-code a tier count. */");
lines.push("  function totalLevels() { return ORDER.length * LEVELS_PER_TIER; }");
lines.push("");
lines.push("  return {");
...
lines.push("    specFor: specFor,");
lines.push("    totalLevels: totalLevels");
lines.push("  };");
lines.push("});");
lines.push("");
